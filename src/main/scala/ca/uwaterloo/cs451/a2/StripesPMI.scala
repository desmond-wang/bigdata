...
object StripesPMI extends Tokenizer {
  ...
  def main(argv: Array[String]) {
    ...
    val pmiCalculation = textFile
      ...
      .map(bigram => (tokenize(bigram)(0),tokenize(bigram)(1)))
      .groupByKey()
      .map(bigram => (bigram._1, bigram._2.groupBy(identity)))
      .map(bigram => (bigram._1, bigram._2.mapValues(v => v.size.toFloat)))
      .sortByKey()
      ...
      .mapPartitions(x => {
        x.map(p => {
          ...
          p._2.foreach(x => {
            var prob_y = single_word_count.value.get(x._1).head
            if (x._2 >= threshold) {
              xy_map(x._1) = (Math.log10(x._2.toFloat * total_word_cout.value/(prob_x.toFloat * prob_y.toFloat)).toFloat, x._2)
            }
          })
          (p._1, xy_map)
        })
